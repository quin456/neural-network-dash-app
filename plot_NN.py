...
def plot_NN(model, input_img=None, s=[784, 16, 10], input_nodes=True):
    ...
    node_vals = (
        model.get_node_vals(input_img, input_nodes) if model is not None else None
    )
    if model is not None:
...
def plot_opt_progress(data):
    fig = go.Figure()
    n = len(data["loss"])
    if n < 2:
        return fig
    x = np.arange(0, n, 1)
    fig.add_trace(go.Scatter(x=x, y=data["loss"], name="Loss"))
    fig.add_trace(go.Scatter(x=x, y=data["accuracy"], name="Accuracy"))
    return fig


def test_plot_opt_progress():
    model = MLP(s=[784, 16, 10])
    process, queue = train_NN_process(model, n_epochs=1, batch_size=256, lr=0.01)
    ...
    data = {key: [data0[key]] for key in data0.keys()}
    data["n_intervals"] = 0
    data["train_model_n_clicks"] = 0

    while process.is_alive():
        if not queue.empty():
            status = queue.get()
            for key in status.keys():
                data[key].append(status[key])
            print(status)
    process.join()


def test_plot_opt_progress_file():
    import json

    with open("data.json", "r") as f:
        data = json.load(f)
    plot_opt_progress(data).show()


if __name__ == "__main__":
    test_plot_opt_progress()
